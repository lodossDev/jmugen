...
//	 zeroed at	 start (size =2*channel_count)
	...
	public int decodeAdxStandard(OutputStream buffer, int samples_needed,
		...
		// file is reached
		while (samples_needed != 0 && sample_index < header.getSampleCount()) {
			...
			for (; sample_offset < sample_endoffset; ++sample_offset, ++sample_index, --samples_needed) {
				for (int i = 0; i < header.getChannelCount(); ++i) {
					...
					file.seek(started_at + header.getBitsPerSample()
					...
					if (header.getBitsPerSample() == 8)
						sample = file.read();
					else if (header.getBitsPerSample() == 16)
						sample = file.readShort();
					else if (header.getBitsPerSample() == 32)
						sample = file.readInt();
					
					
					sample = sign_extend(sample, header.getBitsPerSample());
			...
			// the loop start
			if (loopingEnabled
					&& sample_index == header.getLoopEndSample())
