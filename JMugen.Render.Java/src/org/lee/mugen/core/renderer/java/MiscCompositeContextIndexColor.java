...
public class MiscCompositeContextIndexColor implements CompositeContext {
	...
	public MiscCompositeContextIndexColor(int rule,
						 float alpha,
...
//		System.out.println(srcPix.length + " ------ x =" + x + " w =" + w + " y = " + y0 + " h = " + dstOut.getHeight());
		for (int i = 0; i < srcPix.length;) {
			...
			case MiscCompositeIndexColor.SUBTRACT:
				dstPix[i] = dstPix[i] - srcPix[i];
				if (dstPix[i] < 0)
					dstPix[i] = 0;
				i++;
		...
		dstOut.setPixels(x, y0, w, dstOut.getHeight(), dstPix);
		
			
	}
	public void compose2(Raster src, Raster dstIn, WritableRaster dstOut) {
		float a=0, ac=0;
		float alpha = this.alpha;
		int t;
		...
		switch ( rule ) {
		case MiscCompositeIndexColor.HUE:
		case MiscCompositeIndexColor.SATURATION:
		case MiscCompositeIndexColor.VALUE:
		case MiscCompositeIndexColor.COLOR: 
			sHsv = new float[3];
			diHsv = new float[3];
			doHsv = new float[3];
			break;
		...
		for ( int y = y0; y < y1; y++ ) {
			...
			while ( i < end ) {
				...
				case MiscCompositeIndexColor.DIFFERENCE:
					dor = dir - sr;
					if ( dor < 0 )
						dor = -dor;
					dog = dig - sg;
				...
				case MiscCompositeIndexColor.MULTIPLY:
					t = dir * sr + 0x80;
					dor = ((t >> 8) + t) >> 8;
					t = dig * sg + 0x80;
					dog = ((t >> 8) + t) >> 8;
				...
				case MiscCompositeIndexColor.SCREEN:
					...
					dob = 255 - ( ((t >> 8) + t) >> 8 );
					break;
				case MiscCompositeIndexColor.SCREEN_PLUS:
					final float percent = 1.9f;
					t = (255-dir) * (255-sr) + 0x80;
					dor = 255 - ( ((t >> 8) + t) >> 8 );
					dor = (int) (dor * percent);
					dor = dor > 255? 255: dor;
					...
					break;
				case MiscCompositeIndexColor.OVERLAY:
					if ( dir < 128 ) {
						t = dir * sr + 0x80;
						dor = 2 * (((t >> 8) + t) >> 8);
					} else {
				...
				case MiscCompositeIndexColor.DARKEN:
					dor = dir < sr ? dir : sr;
					dog = dig < sg ? dig : sg;
					dob = dib < sb ? dib : sb;
					break;

				case MiscCompositeIndexColor.LIGHTEN:
					dor = dir > sr ? dir : sr;
					dog = dig > sg ? dig : sg;
					dob = dib > sb ? dib : sb;
					break;

				case MiscCompositeIndexColor.AVERAGE:
					dor = (dir + sr) / 2;
					dog = (dig + sg) / 2;
					dob = (dib + sb) / 2;
					break;
				...
				case MiscCompositeIndexColor.BURN:
					if (dir != 255)
				...
				case MiscCompositeIndexColor.COLOR_BURN:
					if (sr != 0)
				...
				case MiscCompositeIndexColor.DODGE:
					dor = clamp((sr << 8) / (256-dir));
					dog = clamp((sg << 8) / (256-dig));
					dob = clamp((sb << 8) / (256-dib));
					break;

				case MiscCompositeIndexColor.COLOR_DODGE:
					if (sr != 255)
				...
				case MiscCompositeIndexColor.SOFT_LIGHT:
					int d;
				...
				case MiscCompositeIndexColor.HARD_LIGHT:
					if (sr > 127)
				...
				case MiscCompositeIndexColor.PIN_LIGHT:
