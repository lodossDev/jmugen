...
public final class MiscCompositeIndexColor implements Composite {
	...
	public final static int SCREEN_PLUS = 25;
	
	private static final int MIN_RULE = BLEND;
	private static final int MAX_RULE = SCREEN_PLUS;
	...
	private MiscCompositeIndexColor(int rule) {
		this(rule, 1.0f);
	}

	private MiscCompositeIndexColor(int rule, float alpha) {
	...
	public static Composite getInstance(int rule, float alpha) {
		switch ( rule ) {
		case MiscCompositeIndexColor.BLEND:
			return AlphaComposite.getInstance( AlphaComposite.SRC_OVER, alpha );
		case MiscCompositeIndexColor.STENCIL:
			return AlphaComposite.getInstance( AlphaComposite.DST_IN, alpha );
		case MiscCompositeIndexColor.SILHOUETTE:
			return AlphaComposite.getInstance( AlphaComposite.DST_OUT, alpha );
		}
		return new MiscCompositeIndexColor(rule, alpha);
	}

	public CompositeContext createContext(ColorModel srcColorModel, ColorModel dstColorModel, RenderingHints hints) {
		return new MiscCompositeContextIndexColor( rule, extraAlpha, srcColorModel, dstColorModel );
	}
	...
	public boolean equals(Object o) {
		if (!(o instanceof MiscCompositeIndexColor))
			return false;
		MiscCompositeIndexColor c = (MiscCompositeIndexColor)o;
