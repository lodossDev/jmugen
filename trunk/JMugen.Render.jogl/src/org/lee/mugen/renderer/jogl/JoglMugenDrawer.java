...
public class JoglMugenDrawer extends MugenDrawer {
	...
	private void dealocateBuffer(ByteBuffer b) {
//		 try {
//		 Field privateStringField = ByteBuffer.class.getDeclaredField("hb");
//		 privateStringField.setAccessible(true);
//		 privateStringField.set(b, new byte[0]);
//		 } catch (Exception e) {
//		 // TODO: handle exception
//		 }
	}

	private Texture backend;
	private Texture backend2;

	public Texture getBackend() {
		if (backend == null) {
			backend = TextureIO.newTexture(new BufferedImage(640, 480,
					BufferedImage.TYPE_INT_ARGB_PRE), false);
			backend.setTexParameteri(GL.GL_TEXTURE_WRAP_S, GL.GL_CLAMP_TO_EDGE);
			backend.setTexParameteri(GL.GL_TEXTURE_WRAP_T, GL.GL_CLAMP_TO_EDGE);
			backend.setTexParameteri(GL.GL_TEXTURE_MAG_FILTER, GL.GL_NEAREST);
			backend.setTexParameteri(GL.GL_TEXTURE_MIN_FILTER, GL.GL_NEAREST);
		}
		return backend;
	}

	public Texture getBackend2() {
		if (backend2 == null) {
			backend2 = TextureIO.newTexture(new BufferedImage(640, 480,
					BufferedImage.TYPE_INT_ARGB_PRE), false);
			backend2
					.setTexParameteri(GL.GL_TEXTURE_WRAP_S, GL.GL_CLAMP_TO_EDGE);
			backend2
					.setTexParameteri(GL.GL_TEXTURE_WRAP_T, GL.GL_CLAMP_TO_EDGE);
			backend2.setTexParameteri(GL.GL_TEXTURE_MAG_FILTER, GL.GL_NEAREST);
			backend2.setTexParameteri(GL.GL_TEXTURE_MIN_FILTER, GL.GL_NEAREST);
		}
		return backend2;
	}
	...
	private static Map<String, BlendShader> blendShaderMap = new HashMap<String, BlendShader>();
	...
	private void drawWithPropertiesColor(DrawProperties dp) {
		...
		if (trans == Trans.ADD) {
			...
			gl
					.glTexEnvf(GL.GL_TEXTURE_ENV, GL.GL_COMBINE_ALPHA,
							GL.GL_SUBTRACT);
			gl.glTexEnvf(GL.GL_TEXTURE_ENV, GL.GL_SRC0_RGB, GL.GL_PREVIOUS);
			gl.glTexEnvf(GL.GL_TEXTURE_ENV, GL.GL_SRC1_RGB, GL.GL_TEXTURE);
		...
		if (dp.getPalfx() != null) {
			...
			getAfterImageShader().render(gl,
					...
					dp.getImageProperties().getPalmul(), type, 2f);
			drawImage(xlDst, xrDst, ytDst, ybDst, xlSrc, xrSrc, ytSrc, ybSrc,
					dp);
	...
	public void draw(DrawProperties dp) {
		...
		if (dp.getIc().getImg() instanceof BufferedImage) {
			...
			TextureRenderer textRender = ((JoglMugenDrawer) GraphicsWrapper
			...
			synchronized (this) {
				if (imageStatus.get() == TEXTURE) {
					...
					try {
						...
						Texture texture = TextureIO
								.newTexture((TextureData) img);
						dealocateBuffer((ByteBuffer) ((TextureData) img)
								.getBuffer());
						...
						throw new IllegalStateException(
								"Ca ne devrait pas arrive", e);
					}
				} else if (imageStatus.get() == RAW_PCX) {
					RawPCXImage pcx = (RawPCXImage) img;
					try {
						BufferedImage image = (BufferedImage) PCXLoader
								.loadImageColorIndexed(
										new ByteArrayInputStream(pcx.getData()),
										pcx.getPalette(), false, true);
						TextureData data = TextureIO.newTextureData(image,
								GL.GL_COMPRESSED_RGBA_S3TC_DXT3_EXT, GL.GL_RGB,
								false);
						// TextureData data = TextureIO.newTextureData(image,
						// false);

						Texture texture = TextureIO.newTexture(data);
						dealocateBuffer((ByteBuffer) data.getBuffer());
		...
		public void prepareImageToTexture() {
			...
			synchronized (this) {
				if (imageStatus.get() == RAW_PCX) {
					...
					try {
						BufferedImage image = (BufferedImage) PCXLoader
								.loadImage(new ByteArrayInputStream(pcx
										.getData()), pcx.getPalette(), false,
										true);
						TextureData data = TextureIO.newTextureData(image,
								GL.GL_COMPRESSED_RGBA_S3TC_DXT3_EXT, GL.GL_RGB,
								false);
	...
	public ImageContainer getImageContainer(Object imageData) {
		if (imageData instanceof RawPCXImage) {
			...
			ImageContainerText result = new ImageContainerText(pcx, width,
					height);
	...
	public ImageContainer getImageContainer(Object imageData, int colors) {
		...
		ImageContainerText result = new ImageContainerText(colors, pcx, width,
				height);
		addToImageToProcess(result);
		return result;
