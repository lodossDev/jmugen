...
public class PCXLoader {
	...
	public static Image loadImageColorIndexed(InputStream file, PCXPalette pal,
		...
		IndexColorModel icm = new IndexColorModel(8, 256,
				convertToByteArray(pal.r), convertToByteArray(pal.g),
				convertToByteArray(pal.b), 0);
		...
		while (yp < height) {
			...
			for (int i = 0; i < count; i++) {
				if (xp < width) {
					...
					int color = ImageUtils.getARGB(alpha[0], pal.r[index],
							pal.g[index], pal.b[index]);

					if (value == 0) {
						color = ImageUtils.getARGB(alpha[0], pal.r[0],
								pal.g[0], pal.b[0]);
	 ...
	 * 
	 * @param file
	 *            The input stream to load the PCX from
	 */
	public static Image loadImage(InputStream file, PCXPalette pal,
		...
		//		
		// image = new BufferedImage(glAlphaColorModel, raster, false, new
		// Hashtable());
		...
		while (yp < height) {
			...
			for (int i = 0; i < count; i++) {
				if (xp < width) {
					...
					if (isUseColorKey)
						isAboutTheSameColor(pal.r[value], pal.g[value],
								pal.b[value], pal.r[0], pal.g[0], pal.b[0],
								alpha);
					int color = ImageUtils.getARGB(alpha[0], pal.r[value],
							pal.g[value], pal.b[value]);
					image.setRGB(xp, yp, color);
		...
		if (isFlipH || isFlipV) {
			result = new BufferedImage(glAlphaColorModel, raster, false,
					new Hashtable());
			Graphics2D g = (Graphics2D) result.getGraphics();
			g.scale(isFlipH ? -1 : 1, isFlipV ? -1 : 1);
			g.drawImage(image, isFlipH ? -width : 0, isFlipV ? -height : 0,
					null);
	...
	private static boolean isAboutTheSameColor(int r1, int g1, int b1, int r2,
	...
	public static byte[] LoadPCX(RawPCXImage pcxRaw, byte[][] outPalette,
			Dimension outDim) {
