...
public class Projcontact extends SpriteCnsTriggerFunction {

	// TODO : Projcontact
	public Projcontact() {
		super("projcontact", new String[] {});
	}
	...
	/*
	ProjContact[ID] = valeur
	ProjContact[ID] = valeur, [oper] valeur2
	 */
	...
	public int parseValue(String[] tokens, int pos, List<Valueable> result) {
		...
		Matcher m = PROJ_CONTACT_SPEC_REG.matcher(keyStr);
...
//				} else 
				if (projDefineCount == projCount) {
					...
					for (ProjectileSub projectile: projectiles) {
						if (projectile.getLastTimeHitSomething() == -1)
							continue;
						long delta = currentTime - projectile.getLastTimeHitSomething();
	...
	public String getRegex() {
		return PROJ_CONTACT_SPEC_REG.pattern();
