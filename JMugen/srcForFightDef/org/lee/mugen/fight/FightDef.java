...
public class FightDef implements AccessorParser {
	...
	public static void main(String[] args) throws Exception {
		FightDef bean = new FightDef();
		String text = IOUtils.toString(new FileInputStream("resource/data/fight.def"));
//		String text = IOUtils.toString(new FileInputStream("resource/data/mvc_lifebars.def"));
		
		Parser.fillBean(bean, Parser.getGroupTextMap(text));
		
	}
	public void parse(GroupText grp) throws Exception {
		if ("files".equalsIgnoreCase(grp.getSection())) {
			files.parse(grp);
		} else if ("lifebar".equalsIgnoreCase(grp.getSection())) {
			lifebar.parse(grp);
		} else if ("powerbar".equalsIgnoreCase(grp.getSection())) {
			powerbar.parse(grp);
		} else if ("face".equalsIgnoreCase(grp.getSection())) {
			face.parse(grp);
	...
	public SpriteAnimManager getAnim() {
		return anim;
	}

	public void setAnim(SpriteAnimManager anim) {
		this.anim = anim;
	}
...
//		String fightDef = Parser.getExistFile("mvc_lifebars.def");
		...
		anim = new SpriteAnimManager("Fight.def", airParser);
		
		Parser.fillBean(this, list);
	}
